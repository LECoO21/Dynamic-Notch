...
本文件记录灵动刘海的重要功能、修复与发布变更。尚未发布的改动统一进入「未发布」，创建新版本标签时再归档到对应版本。

## [未发布]

### 修复

- 移除展开工作区顶部的全部透明占位，白色底板改为紧贴屏幕物理最上沿，并完全遮盖面板范围内的菜单栏；卡片区域大小及折叠刘海尺寸保持不变，点击收起区域移到两组导航之间。
- 修复收起动画只等待内容淡出便提前缩小原生窗口的问题；现在由轻量底板完整形变回刘海后再切换窗口，消除末尾卡顿和大面积内容栅格化。
- 修复动画中内容保持原尺寸、越过正在缩放的白色底板形成重影的问题；展开后半程内容会随底板放大渐显，收起前半程内容会同步缩小淡出，并在形变一半时完全隐藏。

### 变更

- 首页卡片的尺寸调整按钮默认隐藏，仅在鼠标悬停对应卡片或键盘聚焦按钮时显示。
- 恢复点击或按 Space 时白色底板在 200px 刘海轮廓与完整工作区之间的双向缓入缓出形变，展开和收起都不再硬切。
- 产品显示名称由「TO-DO Panel」更新为「灵动刘海」；保留原有应用身份、数据目录、存储键与 GitHub 仓库地址，确保已有本地数据和发布链接继续兼容。
- 桌面应用与任务提醒图标改用官网宣传图中的银白色灵动刘海标志。
- 桌面端整体改为黑色顶栏、暖白内容层与浅色卡片的新版界面，并统一各页面的间距、按钮和文字层级。
- 顶部 Tab 指示器改为无底边台阶的清晰矢量曲线，顶栏与首行卡片之间的留白加倍，并移除顶栏左右两侧的品牌与设备状态块。
- 首页移除镜子及全部摄像头权限、设置和运行逻辑；其余六个组件自动铺满布局，并保留旧用户已有的排序与尺寸偏好。
