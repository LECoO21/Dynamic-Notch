...
- 展开态布局：黑幕从屏幕最顶垂下、盖住菜单栏带（与刘海连为一体，只圆下方两角）；顶栏下方安全带只保留居中的 Tab，不显示左右品牌或设备状态块。
...
- 展开/收起交互：折叠态点折叠条展开（无唇边，靠 screen-saver 级窗口穿透菜单栏拦截）；收起 = 点击顶栏中 Tab 以外的黑色空白 / 点面板外任意处（窗口失焦）自动收起 / Esc（主进程 before-input-event 转发兜底，Escape 不会原生到达页面）
