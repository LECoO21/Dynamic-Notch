...
灵动刘海是一个常驻 macOS 屏幕顶部的本地工作台。默认折叠成物理刘海大小，点击后从顶部展开；常用信息和动作不必再散落在多个应用里。
...
| **设置** | 当菜单栏图标被刘海遮挡时，仍可在面板内配置 API、首页组件、功能显示、快捷键、数据目录与开机启动 |
...
「设置 → 首页组件」可以隐藏或恢复首页的六个组件，但首页至少保留一个。隐藏后，其余组件会自动重新铺满整个 Bento 网格，不留空洞；组件数据、用户保存的排列顺序和尺寸偏好不会被删除或覆盖。只要存在隐藏组件，首页使用自动填充布局并暂时隐藏尺寸按钮；恢复全部六个组件后，原尺寸偏好与按钮会一并恢复。该偏好随当前本地工作区保存，存储键为 `notch-home-hidden-modules-v1`。
...
- **贴顶但不打扰**：折叠态宽 200px，高度跟随菜单栏；展开后的白色底板从当前屏幕最上沿开始，以不透明底色和 `screen-saver` 窗口层级覆盖面板范围内的菜单栏。各页共用 `1240 × 540` 展开窗口，不再预留菜单栏高度；窄屏与矮屏保留 `24px` 安全距。两组导航之间的空白可点击收起，切屏仍贴物理顶端居中；展开与通知都不使用系统窗口动画。白色底板以渲染层缓动在 200px 刘海轮廓与完整工作区之间形变；展开后半程内容随底板放大渐显，收起前半程内容随底板缩小淡出，到一半时完全隐藏，避免越过底板产生重影。
- **设备按需启用**：麦克风只有主动开始录音时才启用，结束录音或退出应用时立即释放。
...
灵动刘海只在 `127.0.0.1:43821` 监听通知接口，来源限 `codex`、`claude` 与 `gpt`：
...
```text
/Applications/灵动刘海.app/Contents/Resources/app/scripts/codex-notify.js
/Applications/灵动刘海.app/Contents/Resources/app/scripts/claude-notify.js
```
