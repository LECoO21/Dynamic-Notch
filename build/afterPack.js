...
exports.default = async function afterPack(context) {
  ...
  // iCloud Drive / Finder 可能会给依赖和构建产物附加 FinderInfo、resource fork、
  // quarantine 或 provenance 等扩展属性。codesign 会把这些属性视为不安全的
  // bundle 污染并拒绝签名，因此只在临时构建产物上递归清理后再签名。
  ...
  const cs = (file, executable = false) => {
    try {
      // FileProvider can reattach Finder metadata while a CloudDocs build is running,
      // so clean each nested signing target immediately before invoking codesign too.
      execFileSync('/usr/bin/xattr', ['-cr', file], { stdio: 'pipe' });
