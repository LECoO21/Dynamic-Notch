...
// Keep the historical internal app name and data directory so upgrading users retain
// safeStorage access, notes, links, recordings and settings after the public rename.
...
let blurCollapseDeferred = false;
...
// 菜单栏高度用于折叠刘海；展开工作区以 screen-saver 层级覆盖此区域。
...
// 原生窗口和白色底板均从屏幕最上沿开始，展开时不再预留菜单栏带。
...
function applyMode(mode, display) {
  ...
  const targetDisplay = display || getWindowDisplay();
  cancelCollapseWatchdog();
  mainWindow.setBounds(getBoundsForMode(mode, targetDisplay));
  if (!mainWindow.webContents.isDestroyed()) {
    mainWindow.webContents.send('window:metrics-changed', getLayoutMetrics(targetDisplay));
  }
...
function repositionWindow(display) {
  if (!mainWindow || mainWindow.isDestroyed()) return;
  const targetDisplay = display || getWindowDisplay();
  mainWindow.setBounds(getBoundsForMode(currentMode, targetDisplay));
  if (!mainWindow.webContents.isDestroyed()) {
    mainWindow.webContents.send('window:metrics-changed', getLayoutMetrics(targetDisplay));
  }
}
...
ipcMain.handle('media:microphone', async () => {
  ...
  try {
    ...
    if (mediaPermissionRequests === 0 && blurCollapseDeferred) {
      blurCollapseDeferred = false;
    }
  }
});
...
async function promptForMissingPermissions() {
  ...
  const { response, checkboxChecked } = await dialog.showMessageBox({
    type: 'info',
    message: `灵动刘海需要「${names.join('」和「')}」权限`,
    detail: [
  ...
  // 否则用户打开设置面板可能找不到灵动刘海这一项、只能手动拖进去。
