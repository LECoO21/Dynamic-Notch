...
const PANEL_MOTION_FALLBACK_MS = 480;
...
function syncPanelAccessibility(expanded) {
  ...
  notch.setAttribute('aria-label', expanded ? '收起灵动刘海' : '展开灵动刘海');
...
// 原生窗口提供透明画布；展开只显示内层白色底板。
// 收起等底板淡出后再缩原生窗口，避免最后一帧被裁掉。
...
function applyLayoutMetrics(metrics) {
  ...
  if (Number.isFinite(metrics.stripHeight) && metrics.stripHeight > 0) {
    document.documentElement.style.setProperty('--notch-h', `${metrics.stripHeight}px`);
  }
  if (Number.isFinite(metrics.menuBarHeight) && metrics.menuBarHeight >= 0) {
...
function positionIndicator() {
  ...
  const groupChanged = tabIndicator.parentElement !== group;
  if (groupChanged) {
    tabIndicator.classList.add('is-relocating');
    group.appendChild(tabIndicator);
  }
  tabIndicator.style.width = `${btn.offsetWidth}px`;
  tabIndicator.style.transform = `translateX(${btn.offsetLeft}px)`;
  if (groupChanged) {
    // Commit the new group-local position before restoring sliding transitions.
    void tabIndicator.offsetWidth;
    tabIndicator.classList.remove('is-relocating');
  }
}
...
// 透明顶栏占位移除后，由底板上两组导航之间的空白接替点击收起。
// 只处理底板/页面背景，卡片和按钮的点击继续交给各自功能。
const navigationBoard = document.getElementById('panels');
if (navigationBoard) {
  navigationBoard.addEventListener('click', (e) => {
    if (e.target !== navigationBoard && !e.target.classList.contains('tab-panel')) return;
    const boardTop = navigationBoard.getBoundingClientRect().top;
    const navBottom = document.getElementById('tabs').getBoundingClientRect().bottom;
    if (e.clientY < boardTop || e.clientY > navBottom) return;
    e.stopPropagation();
    setMode(false);
  });
}
...
function loadHomeOrder() {
  try {
    ...
    const saved = Array.isArray(rawSaved)
      ...
      : rawSaved;
    const currentSaved = Array.isArray(saved)
      ? saved.filter((id) => HOME_ORDER_DEFAULTS.includes(id))
      : saved;
    if (
      Array.isArray(currentSaved)
      && currentSaved.length === HOME_ORDER_DEFAULTS.length
      && new Set(currentSaved).size === HOME_ORDER_DEFAULTS.length
    ) return currentSaved;
