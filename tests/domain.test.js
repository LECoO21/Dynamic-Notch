...
test('home layout swaps complete slot assignments without duplicates', () => {
  ...
  assert.deepEqual(swapHomeLayoutSlots(defaults, 'music', 'clock'), {
    windows: 'tall-left',
    clock: 'square-top',
    recorder: 'medium-top',
    music: 'small-top',
...
test('home widget sizes fill the complete bento capacity without blank cells', () => {
  ...
  const fitted = normalizeHomeWidgetSizes({ ...defaults, music: 'large' }, defaults, 'music', 48);
...
test('hidden homepage modules are deduplicated and normalized to module order', () => {
  assert.deepEqual(
    normalizeHiddenHomeModules(['note', 'unknown', 'note', 'music'], HOME_MODULES),
    ['music', 'note']
  );
...
test('five-widget layout chooses the largest preference and breaks ties by saved order', () => {
  ...
  const layout = resolveHomeWidgetLayout(order, sizes, ['pomodoro'], 12, 4);
...
test('home layout validation rejects every incomplete or unsafe shape', () => {
  const valid = resolveHomeWidgetLayout(
    ['music', 'windows', 'commands'],
    { music: 'large', windows: 'large', commands: 'mini' },
    ['commands'],
    12,
    4
  );
